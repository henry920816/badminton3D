...
objectDefineProperties.f = DESCRIPTORS$b && !V8_PROTOTYPE_DEFINE_BUG ? Object.defineProperties : function defineProperties(O, Properties) {
  ...
  var index = 0;
  var key;
  while (length > index) definePropertyModule$2.f(O, key = keys4[index++], props[key]);
  return O;
};
...
var getJsonReplacerFunction = function(replacer) {
  ...
  for (var i = 0; i < rawLength; i++) {
    ...
    if (typeof element == "string") push$9(keys4, element);
    else if (typeof element == "number" || classof$8(element) === "Number" || classof$8(element) === "String") push$9(keys4, toString$4(element));
  }
...
$$q({ target: "Array", proto: true, forced: !HAS_SPECIES_SUPPORT }, {
  splice: function splice(start, deleteCount) {
    ...
    for (k = 0; k < actualDeleteCount; k++) {
      from3 = actualStart + k;
      if (from3 in O) createProperty$1(A, k, O[from3]);
    }
...
var objectAssign = !$assign || fails$b(function() {
  ...
  while (argumentsLength > index) {
    ...
    while (length > j) {
      key = keys4[j++];
      if (!DESCRIPTORS$7 || call$7(propertyIsEnumerable3, S, key)) T[key] = S[key];
    }
  }
  return T;
} : $assign;
...
var onFreeze = function(it) {
  if (FREEZING && REQUIRED && isExtensible2(it) && !hasOwn$3(it, METADATA)) setMetadata(it);
  return it;
};
var enable = function() {
